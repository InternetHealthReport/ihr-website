...
import { MetisQuery } from '@/plugins/IhrApi'
  ...
  methods: {
    ...
    setFilter() {
      if (this.metric && this.nbprobes && this.af) {
        this.apiFilter = new MetisQuery().ranking(this.nbprobes).addressFamily(this.af).metric(this.metric).orderedByTime().orderedByRank()
      }
    },
    apiCall() {
      this.fetch = false
