      ...
      <q-tr :props="props">
        ...
        <q-td key="asn" :props="props">
          ...
          >
            {{
              index == 1
                ? "/" + $options.filters.ihr_NumberToAsOrIxp(asn)
                : $options.filters.ihr_NumberToAsOrIxp(asn)
            }}
          </a>
        </q-td>
        ...
        <q-td key="delayChange" :props="props">{{
    ...
    computeDataSummary() {
      ...
      this.data.forEach(alarm => {
        if (alarm.link in datasum) {
          ...
          if (!datasum[alarm.link].asn.includes(alarm.asn))
            datasum[alarm.link].asn.push(alarm.asn);
          ...
          Object.keys(alarm.msm_prb_ids).forEach(msmid => {
            if (msmid in datasum[alarm.link].msm_prb_ids) {
              var union = [
                ...new Set([
                  ...alarm.msm_prb_ids[msmid],
                  ...datasum[alarm.link].msm_prb_ids[msmid]
                ])
              ];
              if (union.length > 0) {
