  ...
  <div class="IHR_chart">
    ...
    <q-card v-if="details.tableVisible" class="bg-accent q-ma-xl" dark>
      ...
      <q-tabs
        ...
        class="table-card text-grey inset-shadow justify"
        indicator-color="secondary"
        active-color="primary"
        active-bg-color="white"
        narrow-indicator
      >
        <q-tab name="dependency" :label="$t('charts.prefixDependencies.table.dependencyTitle')" />
      ...
      <q-tab-panels v-model="details.activeTab" animated>
        ...
        <q-tab-panel name="api" class="IHR_api-table q-pa-lg" light>
          <h3>{{ $t('charts.prefixDependencies.table.apiTitle') }}</h3>
          <table>
            <tr>
              <td>
                <p class="text-subtitle1">
                  {{ $t('charts.prefixDependencies.table.dependencyTitle') }}
                </p>
              </td>
              <td>
...
export default {
  ...
  methods: {
    updateAxesLabel() {
      this.layout.yaxis.title = 'Prefix' + this.prefix + ` ${this.$t('charts.prefixDependencies.yaxis')}`
    },
    makeHegemonyFilter() {
      return new HegemonyPrefixQuery()
        .prefix(this.prefix)
  ...
  computed: {
    bgplay() {
      return `/ihr/widget/bgplay?prefix=${this.prefix}&date=${this.dateStr}`
    },
    dateStr() {
