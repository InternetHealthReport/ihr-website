...
export default {
  ...
  methods: {
    ...
    updateProbes() {
      ...
      this.noData =
        this.probes.length === 0 && !this.loading ? this.$t("noOutage") : false;
    },
  ...
  computed: {
    traces() {
      ...
      this.probes.forEach(prob => {
        ...
        let durationHour = Math.ceil(
          Math.abs(prob.endTime - prob.startTime) / (1000 * 60 * 60)
        );
        var durationMin = Math.ceil(
          Math.abs(prob.endTime - prob.startTime) / (1000 * 60)
        );
        ...
        let probeText = `<b>${prob.label}</b><br> PB${
          prob.id
        }<br> ${this.dateFormatter(
          prob.startTime
