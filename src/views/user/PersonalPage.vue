...
<script>
import SelectSearchBar from "@/views/Select";
...
const ConfirmElement = {
  ...
  computed: {
    primaryButton() {
      return this.value
        ? this.$t("personalPage.change")
        : this.$t("personalPage.confirm");
    },
    primaryColor() {
  ...
  data() {
    ...
    for (let level in MonitoringUserQuery.NOTIFY_LEVEL) {
      ...
      monitoringLevel.push({
        label: MonitoringUserQuery.NOTIFY_LEVEL.toString(
          MonitoringUserQuery.NOTIFY_LEVEL[level]
        ),
...
#settings .row .q-table__bottom {
  display: none;
}

#settings .row .q-table__middle {
  display: none;
}
