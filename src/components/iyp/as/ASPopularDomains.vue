...
const domains = ref({
  ...
  show: false,
  loading: true,
  query: `MATCH (:AS {asn: $asn})-[:ORIGINATE]->(p:BGPPrefix)<-[po:PART_OF]-(:IP)<-[:RESOLVES_TO]-(h:HostName)-[:PART_OF]->(d:DomainName)-[rr:RANK]->(rn:Ranking)
    WHERE rr.rank < 100000 AND rr.reference_name = 'tranco.top1m' AND h.name = d.name AND "BGPPrefix" IN po.prefix_types
