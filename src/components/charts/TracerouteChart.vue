  ...
  <div class="graph-container">
    ...
    >
      <div style="display: flex; align-items: center">
        <span class="nodeTypeDot" :style="{ backgroundColor: tooltipData.color }"></span
        >{{ tooltipData.type }}
      </div>
      <div>
    ...
    <div v-if="displayMode === 'rtt' && Object.keys(nodes).length > 0" class="rtt-info-overlay">
      <div>
        <span class="rtt-dot" :style="{ backgroundColor: rttColor(minDisplayedRtt) }"></span>
        <strong>Min RTT: </strong>{{ minDisplayedRtt ? minDisplayedRtt + ' ms' : 'Not available' }}
      </div>
      <div>
        <span class="rtt-dot" :style="{ backgroundColor: rttColor(maxDisplayedRtt) }"></span>
