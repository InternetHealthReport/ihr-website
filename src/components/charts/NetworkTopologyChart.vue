...
const search = async () => {
  ...
  if (isPrefix(searchInput.value)) {
    const query_params = {
      prefix: searchInput.value,
      af: Number(ipModel.value[ipModel.value.length - 1])
    }
    ...
    const asnInfoResponse = await iyp_api.run([
      {
        statement: as_topology_query.value.query3,
        parameters: { asns: Object.keys(nodeInfo.value).map(Number) }
      }
    ])
    ...
    const prefixInfoResponse = await iyp_api.run([
      { statement: prefix_topology_query.value.query2, parameters: { prefix: searchInput.value } }
    ])
...
const sortNodes = (data) => {
  ...
  Array.from(uniqueNodes).forEach((asn) => {
    sortedUniqueNodes[asn] = {
      name: `AS${asn}`,
      tooltip: { visible: false, left: 0, top: 0 },
      color: ASN.value == asn ? 'red' : calculateNodeColor(nodeInfo.value[asn]?.HEGE),
      size: calculateNodeSize(nodeInfo.value[asn]?.CONES),
      type: 'circle'
    }
  })
  if (isPrefix(searchInput.value) && Object.keys(sortedUniqueNodes).length > 0)
    sortedUniqueNodes[searchInput.value] = {
...
watch(searchInput, (newValue) => {
  ...
  if(newValue.includes('.')) ipModel.value = "IPv4"
  else if(newValue.includes(':')) ipModel.value = "IPv6"
})
...
<template>
  <QCard style="margin-bottom: 15px" :flat="props.isComponent" :bordered="!props.isComponent">
    ...
    <QCardSection class="graphContainer" v-if="!loading && Object.keys(allNodes).length > 0">
      ...
      <div
        ref="tooltip"
        class="tooltipTopology"
        :style="{ ...tooltipPos, opacity: tooltipHoverOpacity }"
      >
        <div>{{ nodeInfo[targetNodeId]?.Name }}</div>
        <div>{{ nodeInfo[targetNodeId]?.Country }}</div>
        <div v-if="!isPrefix(targetNodeId)">
          Customer Cones : {{ nodeInfo[targetNodeId]?.CONES }}
        </div>
        <div v-if="targetNodeId != ASN && !isPrefix(targetNodeId)">
          Hegemony : {{ nodeInfo[targetNodeId]?.HEGE.toFixed(2) }}%
        </div>
      </div>
    </QCardSection>
