...
export function etl(alarms, aggregatedAttrsZipped, countryName, alarmTypeTitlesMap, legend, isASGranularity) {
  ...
  const alarmCountsAggregatedBySeverity = aggregateAlarmCountsBySeverityType(alarmsFilteredByCountryOptional, aggregatedAttrsZipped, asGranularity.key)
  const treeMapTrace = getTreeMapTrace(alarmCountsAggregatedBySeverity, asGranularity.key, alarmTypeTitlesMap, legend)
  return treeMapTrace
}
  ...
  for (const [alarmCountType, _, alarmSevertiyType, [__, ___]] of aggregatedAttrsZipped) {
    for (const alarm of alarms) {
      ...
      for (const severityType in severityTypeAlarmCounts) {
        ...
        const alarmCountAggregatedBySeverity = { [key]: keyValue, alarm_type: alarmTypeTitledCase, severity: severityTypeTitledCase, count: count }
...
function getTreeMapTrace(alarmCountsAggregatedBySeverity, key, alarmTypeTitlesMap, legend) {
  ...
  const keyUniqueValues = AggregatedAlarmsUtils.getPropertyUniqueValues(alarmCountsAggregatedBySeverity, key);
  const alarmTypeUniqueValues = AggregatedAlarmsUtils.getPropertyUniqueValues(alarmCountsAggregatedBySeverity, 'alarm_type');
  const severityUniqueValues = AggregatedAlarmsUtils.getPropertyUniqueValues(alarmCountsAggregatedBySeverity, 'severity');

  const trace = createBaseTrace(legend)

  addKeyValues(trace, alarmCountsAggregatedBySeverity, key, keyUniqueValues);

  addAlarmTypeValues(trace, alarmCountsAggregatedBySeverity, key, keyUniqueValues, alarmTypeUniqueValues, alarmTypeTitlesMap);

  addSeverityValues(trace, alarmCountsAggregatedBySeverity, key, keyUniqueValues, alarmTypeUniqueValues, severityUniqueValues)
...
function addKeyValues(trace, alarms, key, keyUniqueValues) {
  for (const keyValue of keyUniqueValues) {
    const keyAlarmCounts = alarms.filter(alarm => alarm[key] === keyValue).reduce((sum, alarm) => sum + alarm.count, 0);
...
function addAlarmTypeValues(trace, alarms, key, keyUniqueValues, alarmTypeUniqueValues, alarmTypeTitlesMap) {
  for (const keyValue of keyUniqueValues) {
    for (const alarmType of alarmTypeUniqueValues) {
      const traceId = `${keyValue}-${alarmType}`
      const alarmTypeCount = alarms.filter(alarm => alarm[key] === keyValue && alarm.alarm_type === alarmType).reduce((sum, alarm) => sum + alarm.count, 0);
...
function addSeverityValues(trace, alarms, key, keyUniqueValues, alarmTypesUniqueValues, severityUniqueValues) {
  for (const keyValue of keyUniqueValues) {
    for (const alarmType of alarmTypesUniqueValues) {
      for (const severity of severityUniqueValues) {
        const alarm = alarms.find(alarm => alarm[key] === keyValue && alarm.alarm_type === alarmType && alarm.severity === severity);
        if (alarm) {
...
export function getChartTitle(treeMapTrace, severitiesSelected, countryName, legend, isASGranularity) {
