...
function extractAlarms(entityType, entityValue, startDateTime, endDateTime, sourceParams) {
  ...
  const request = IodaApiPlugin.getIodaEntityInfo(
    entityType,
    entityValue,
    startUnixTime,
    endUnixTime,
...
function transformAlarms(iodaData, iodaAlarmTypes) {
  ...
  for (const iodaAlarmTypeData of iodaData) {
    ...
    const unitValuesTransformed = iodaAlarmTypeData.values
      .filter((val) => val !== null)
      .map((unitValue) =>
        AggregatedAlarmsUtils.getPercentageValue(unitValue, maxAlarmTypeUnitvalue)
      )
    ...
    while (fromUnixTime <= untilUnixTime) {
      if (iodaAlarmTypeData.values[index] !== null) {
        ...
        datesTransformed.push(
          `${dateTimeObj.getUTCFullYear()}-${('0' + (dateTimeObj.getUTCMonth() + 1)).slice(-2)}-${('0' + dateTimeObj.getUTCDate()).slice(-2)}T${('0' + dateTimeObj.getUTCHours()).slice(-2)}:${('0' + dateTimeObj.getUTCMinutes()).slice(-2)}:${('0' + dateTimeObj.getUTCSeconds()).slice(-2)}.000Z`
        )
      }
    ...
    if (
      datesTransformed.length !== 0 &&
      unitValuesTransformed.length !== 0 &&
      datesTransformed.length === unitValuesTransformed.length
    ) {
...
export function getIodaAlarmTypesUnits() {
  ...
  for (const iodaAlarmType in ALARMS_INFO.ioda.alarm_types) {
    iodaAlarmTypesUnitsResult[iodaAlarmType] =
      ALARMS_INFO.ioda.alarm_types[iodaAlarmType].metadata.unit
  }
