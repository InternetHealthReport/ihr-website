...
function extractAlarms(entityType, entityValue, startDateTime, endDateTime, sourceParams) {
  const startUnixTime = Math.floor(Date.UTC(startDateTime.getUTCFullYear(), startDateTime.getUTCMonth(), startDateTime.getUTCDate(), startDateTime.getUTCHours(), startDateTime.getUTCMinutes(), startDateTime.getUTCSeconds()) / 1000)
  const endUnixTime = Math.floor(Date.UTC(endDateTime.getUTCFullYear(), endDateTime.getUTCMonth(), endDateTime.getUTCDate(), endDateTime.getUTCHours(), endDateTime.getUTCMinutes(), endDateTime.getUTCSeconds()) / 1000)
  ...
  for (const iodaAlarmTypeData of iodaData) {
    ...
    while (fromUnixTime <= untilUnixTime) {
      if (iodaAlarmTypeData.values[index] !== null) {
        ...
        datesTransformed.push(`${dateTimeObj.getUTCFullYear()}-${("0" + (dateTimeObj.getUTCMonth() + 1)).slice(-2)}-${("0" + dateTimeObj.getUTCDate()).slice(-2)}T${("0" + dateTimeObj.getUTCHours()).slice(-2)}:${("0" + dateTimeObj.getUTCMinutes()).slice(-2)}:${("0" + dateTimeObj.getUTCSeconds()).slice(-2)}.000Z`)
      }
