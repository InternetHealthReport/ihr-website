...
jest.mock('../IodaApi') // Mocking the IodaApiPlugin module
...
describe('getIodaAlarms', () => {
  ...
  it(
    'returns valid JSON data with expected schema',
    async () => {
...
describe('getIodaEntityInfo', () => {
  ...
  beforeAll(async () => {
    ...
    const entityType1 = 'asn'
    const entityValue1 = '59127'
    const sourceParams1 = 'WEB_SEARCH'
    const entityType2 = 'country'
    const entityValue2 = 'US'
    const sourceParams2 = 'WEB_SEARCH'
  ...
  it.each([
    ['asn', asnIodaEntityInfo],
    ['country', countryIodaEntityInfo],
  ])(
    'returns valid JSON data with expected schema for entity type %s',
    (entityType, entityInfo) => {
      ...
      try {
        const result = expectedSchema.validate(entityInfo)
  ...
  it(
    'checks alarm values within the time range',
    () => {
      ...
      for (const alarmTypeData of alarmTypesData) {
        ...
        let untilUnixTime = alarmTypeData.form + alarmTypeData.nativeStep
        let counter = 0
